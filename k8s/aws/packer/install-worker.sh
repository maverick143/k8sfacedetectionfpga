...
#sudo bash -c "/sbin/iptables-save > /etc/iptables.conf"
#
...
#wget -O $xrt_package "https://www.xilinx.com/bin/public/openDownload?filename=$xrt_package"
#sudo apt install -y ./${xrt_package}

###
### Remove uwsgi from systemd
###
...
sudo mv $TEMPLATE_DIR/fpga-init.sh /usr/local/bin/fgpa-init.sh
sudo chmod +x /usr/local/bin/fgpa-init.sh
sudo mv $TEMPLATE_DIR/fpga.service /etc/systemd/system/fpga.service
sudo chown root:root /etc/systemd/system/fpga.service
sudo chmod 664 /etc/systemd/system/fpga.service
sudo systemctl daemon-reload
sudo systemctl enable fpga.service
